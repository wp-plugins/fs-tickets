...
function fst_tickets_page() {
	global $wpdb,$fstconfig,$FSTPages,$user_ID;
	...
	if (isset($_POST['ticketid'])) {
		...
		$wpdb->query("UPDATE ".$wpdb->prefix."fst_tickets SET ticket_last_modified = NOW() WHERE ticket_id = ".$_POST['ticket_id']);
	}
	...
	if (isset($_GET['tid'])) {
		...
		if (isset($_POST['submit'])) {
			$wpdb->query("UPDATE ".$wpdb->prefix."fst_tickets SET ticket_status = '".$_POST['ticket_status']."' WHERE ticket_id = ".$_GET['tid']);
			$wpdb->query("UPDATE ".$wpdb->prefix."fst_tickets SET ticket_last_modified = NOW() WHERE ticket_id = ".$_GET['tid']);
			if ($_POST['message_content'] != '') {
				...
				$wpdb->query("UPDATE ".$wpdb->prefix."fst_tickets SET ticket_last_modified = NOW() WHERE ticket_id = ".$TicketDetails->ID);
	...
	if ($TicketPage == 'pending') {
		$Tickets = $wpdb->get_results("SELECT *, DATE_FORMAT(ticket_date_added, '%b %d %Y - %r') as DATEADDED, DATE_FORMAT(ticket_last_modified, '%b %d %Y - %r') as LASTMODIFIED FROM ".$wpdb->prefix."fst_tickets WHERE ticket_status = 'pending' ORDER BY ticket_last_modified DESC");
	} elseif ($TicketPage == 'pendinguser') {
		$Tickets = $wpdb->get_results("SELECT *, DATE_FORMAT(ticket_date_added, '%b %d %Y - %r') as DATEADDED, DATE_FORMAT(ticket_last_modified, '%b %d %Y - %r') as LASTMODIFIED FROM ".$wpdb->prefix."fst_tickets WHERE ticket_status = 'pendinguser' ORDER BY ticket_last_modified DESC");
	} elseif ($TicketPage == 'resolved') {
		$Tickets = $wpdb->get_results("SELECT *, DATE_FORMAT(ticket_date_added, '%b %d %Y - %r') as DATEADDED, DATE_FORMAT(ticket_last_modified, '%b %d %Y - %r') as LASTMODIFIED FROM ".$wpdb->prefix."fst_tickets WHERE ticket_status = 'resolved' ORDER BY ticket_last_modified DESC");
	} else {
		$Tickets = $wpdb->get_results("SELECT *, DATE_FORMAT(ticket_date_added, '%b %d %Y - %r') as DATEADDED, DATE_FORMAT(ticket_last_modified, '%b %d %Y - %r') as LASTMODIFIED FROM ".$wpdb->prefix."fst_tickets ORDER BY ticket_last_modified DESC");
	}
	...
	echo '<table class="widefat page fixed" cellspacing="0" bTicket="1">
		...
		<th scope="col" class="manage-column" width="220">Date</th>
		<th scope="col" class="manage-column">Ticket Overview</th>
		<th scope="col" class="manage-column" width="75">Messages</th>
		<th scope="col" class="manage-column" width="150">Last Poster</th>
		</tr>
		</thead>
		<tfoot>
		<tr>
		...
		foreach ($Tickets as $Tickets) {
			$Messages = $wpdb->get_var("SELECT COUNT(*) FROM ".$wpdb->prefix."fst_tickets_messages WHERE ticket_id = ".$Tickets->ticket_id);
			$LastMessageID = $wpdb->get_var("SELECT ID FROM ".$wpdb->prefix."fst_tickets_messages WHERE ticket_id = ".$Tickets->ticket_id." ORDER BY message_id DESC LIMIT 1");
			$LastPoster = $wpdb->get_var("SELECT user_nicename FROM ".$wpdb->prefix."users WHERE ID = ".$LastMessageID);
			echo '<tr>';
			...
			if ($Messages == 1) {
				echo '<td><strong>Added:</strong> '.$Tickets->DATEADDED.'</td>';
			} else {
				echo '<td><strong>Updated:</strong> '.$Tickets->LASTMODIFIED.'<br/><strong>Added:</strong> '.$Tickets->DATEADDED.'</td>';
			}
			echo '<td>'.stripslashes($Tickets->ticket_title).'<br />';
			...
			echo '</td>';
			echo '<td align="center">'.$Messages.'</td>';
			echo '<td>'.$LastPoster.'</td>';
			echo '</tr>';
		}
	echo '</tbody></table>';
	echo '</div>';
